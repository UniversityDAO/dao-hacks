...
contract CryptoDevsDAO {
   ...
   constructor(address ntfContract, address marketplaceContract) payable {
       cryptoDevsNft = ICryptoDevsNFT(ntfContract);
   ...
   struct Member {
       ...
       // Array of tokenIds for CryptoDevs NTF that are locked up by this member
   ...
   function createProposal(uint256 _forTokenId, ProposalType _proposalType) external memberOnly returns (uint256) {
       if (_proposalType == ProposalType.BUY) {
           require(nftMarketplace.available(_forTokenId), "NTF_NOT_FOR_SALE");
       } else {
           require(nftMarketplace.ownerOf(_forTokenId) == address(this), "NOT_OWNED");
       }
   ...
   // Need a way for people to leave the DAO
   function quit() external memberOnly {
       Member storage member = members[msg.sender];
       require(block.timestamp - member.joinedAt > 5 minutes, "MIN_MEMBORSHIP_PERIOD");
       ...
       for (uint i = 0; i < member.lockedUpNFTs.length; i++) {
           cryptoDevsNft.safeTransferFrom(address(this), msg.sender, member.lockedUpNFTs[i], "");
       }
